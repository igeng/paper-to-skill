...
import subprocess
import sys
from paper_to_skill.config import PYTHON_DEPENDENCIES
...
DEPENDENCY_GROUPS = [
    ...
    {
        "label": "PDF (technical: tables, formulas, figures)",
        "modules": ["docling"],
        "any_of_modules": True,
        "system": [],
...
def normalize_install_mode(argv: list[str]) -> str:
    mode = os.environ.get("PAPER_SKILL_INSTALL_MISSING", "ask").lower()
...
def prepare_dependencies(extraction_mode: str, install_mode: str) -> None:
...
def run_dependency_check() -> int:
    print("paper-to-skill — dependency check\n")
    ...
    if not missing_pip_packages and not missing_system:
        print("All optional dependencies are installed. You're ready.")
        return 0

    print("To enable the best extractor, install the missing pieces:\n")
